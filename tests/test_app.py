...
class TestApp:
    ...
    def test_app_get_edges(self):
        flask_app = self.init_app()
        with flask_app.test_client() as test_client:
            response = test_client.post("/getEdges", data={"image": self.test_img_url})
    ...
    def test_app_sharpen(self):
        flask_app = self.init_app()
        with flask_app.test_client() as test_client:
            response = test_client.post("/sharpen", data={"image": self.test_img_url})
            response_data = response.json
            assert isinstance(response_data["img"], list)
    ...
    def test_app_blur(self):
        flask_app = self.init_app()
        with flask_app.test_client() as test_client:
            response = test_client.post("/blur", data={"image": self.test_img_url})
            response_data = response.json
            assert isinstance(response_data["img"], list)
